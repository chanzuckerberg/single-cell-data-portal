...
local-rebuild: .env.ecr local-ecr-login ## Rebuild local dev without re-importing data
	docker-compose $(COMPOSE_OPTS) build reverse_proxy frontend backend processing wmg_processing
	docker-compose $(COMPOSE_OPTS) up -d reverse_proxy frontend backend processing database oidc localstack
