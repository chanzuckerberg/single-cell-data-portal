...
# The following recipies use peotry to manage python dependencies
# https://python-poetry.org/docs/
# update the poetry dependencies for each subdirectory under ./python_dependencies
.PHONY: poetry/update_python_dependecies
poetry/update_python_dependecies:
	@echo "Generating requirements.txt"
	@echo "# This file is generated by make poetry/requirements.txt" > requirements.txt
	@echo "# Do not edit this file directly" >> requirements.txt
...
# lock python dependencies for each subdirectory under ./python_dependencies using poetry
.PHONY: poetry/lock_python_dependecies
poetry/lock_python_dependecies:
	@echo "Generating poetry.lock"
	@for dir in $(shell find python_dependencies -maxdepth 1 -type d -not -path python_dependencies); do \
		if [ -f $$dir/pyproject.toml ]; then \
			echo "Generating lock for $$dir"; \
			cd $$dir; \
			poetry lock; \
			cd -; \
		fi; \
