...
.PHONY: local-backend
local-backend:
	$(MAKE) local-server -C ./backend/api/data_portal/api_server
...
local-uploadjob: .env.ecr ## Run the upload task with a dataset_id and dropbox_url
	docker-compose $(COMPOSE_OPTS) run --rm -T -e DATASET_ID=$(DATASET_ID) -e DROPBOX_URL=$(DROPBOX_URL) processing sh -c "rm -rf /local.* && python3 -m backend.api.data_portal.dataset_processing.process"
