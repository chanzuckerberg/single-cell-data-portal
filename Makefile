...
# update the poetry dependencies for each subdirectory under ./python_dependencies
.PHONY: poetry/update
poetry/update:
	@echo "Updating poetry dependencies"
	@for dir in $(shell find python_dependencies -maxdepth 1 -type d -not -path python_dependencies); do \
...
# go through the ./python_dependecies directory and generate two differennt requirements files in each subdirectory
# under python_dependecies. The first requirements file will be named requirements.txt and include evreything in
# tool.poetry.dependencies. The second requirements file will be name requirement-dev.txt and will inlcude everything
# under tool.poetry.group.dev.dependencies.
.PHONY: poetry/requirements.txt
poetry/requirements.txt: poetry/update
	@echo "Generating requirements.txt"
	@echo "# Do not edit this file directly" >> requirements-dev.txt
	@echo "# This file is generated by make poetry/requirements.txt" > requirements.txt
	@echo "" >> requirements.txt
	@echo "# Do not edit this file directly" >> requirements-dev.txt
	@echo "# This file is generated by make poetry/requirements.txt" > requirements-dev.txt
	@echo "" >> requirements-dev.txt
	@for dir in $(shell find python_dependencies -maxdepth 1 -type d -not -path python_dependencies); do \
		if [ -f $$dir/pyproject.toml ]; then \
		  	cp requirements.txt $$dir/requirements.txt; \
		  	cp requirements-dev.txt $$dir/requirements-dev.txt; \
			cd $$dir; \
			echo "Generating requirements.txt for $$dir"; \
			poetry export -f requirements.txt --without-hashes -o requirements-temp.txt; \
			cat requirements-temp.txt >> requirements.txt; \
			rm requirements-temp.txt; \
			echo "Generating requirements-dev.txt for $$dir"; \
			poetry export -f requirements.txt --without-hashes -o requirements-dev-temp.txt --with dev; \
			cat requirements-dev-temp.txt >> requirements-dev.txt; \
			rm requirements-dev-temp.txt; \
			cd -; \
		fi; \
	done
	# remove all files that start with requirements
