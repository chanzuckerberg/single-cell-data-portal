...
CZ CELLxGENE Annotate is intended to be used by researchers on their local machines. In cases where collaborators are not comfortable installing CellxGene, it is possible to host CellxGene, so your collaborators only need to open the URL you send them.
