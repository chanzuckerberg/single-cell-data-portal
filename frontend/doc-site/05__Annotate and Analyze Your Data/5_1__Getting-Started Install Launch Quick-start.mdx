...
`--diffexp-lfc-cutoff` as explained in [the methods](/docs/04__Analyze%20Public%20Data/4_3__Algorithms%20and%20Ontologies), genes are only returned in differential expression if the effect size is above the specified threshold for log fold change. Defaults to 0.01.
