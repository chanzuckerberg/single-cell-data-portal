...
- **Cell metadata in obs (for ontology term IDs, the values MUST be the most specific term available from the specified ontology)**:
  ...
  - donor_id: free-text identifier that distinguishes the unique individual that data were derived from. ID should be unique within the collection of datasets submitted.
