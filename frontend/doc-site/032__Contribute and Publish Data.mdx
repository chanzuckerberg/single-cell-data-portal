...
- **Collection information**:
  ...
  - Consortia: *optional* can be added later. Can be one or more of those listed [here](https://github.com/chanzuckerberg/single-cell-data-portal/blob/main/backend/layers/common/validation.py#L12)

The full schema is documented [here](https://github.com/chanzuckerberg/single-cell-curation/blob/main/schema/5.2.0/schema.md) but is summarized below. Each dataset needs the following information added to a single h5ad (AnnData 0.10) format file:

- **Dataset-level metadata in uns**:
  - **title**: title of the individual dataset
  - **batch_condition**: *optional* list of obs fields that define “batches” that a normalization or integration algorithm should be aware of
  - **default_embedding**: *optional* the obsm key associated with the embeddings you would like to be displayed in CELLxGENE by default
- **Data in .X and raw.X**:
