...
- **Cell metadata in obs** (ontology term IDs MUST be the most specific term available from the specified ontology):
  ...
  - **development_stage_ontology_term_id**
    ...
    - other organisms [UBERON](https://www.ebi.ac.uk/ols4/ontologies/uberon/classes/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252FUBERON_0000105)
  ...
  - **self_reported_ethnicity_ontology_term_id**
    - should describe the race/ethnicity gathered via self-reporting from donors, clinicians, etc. _not_ genetically-derived ancestry information
    - human [HANCESTRO]
  ...
  - **tissue_ontology_term_id**
    ...
    - roundworm [WBbt](https://www.ebi.ac.uk/ols4/ontologies/wbbt/classes/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252FWBbt_0005766) or [UBERON]
  ...
  - **suspension_type**
    - `cell`, `nucleus`, or `na`
  - **experimental_condition_ontology_term_id** _optional_
    - should describe any known condition/factor thought to, or being tested to, have an impact on the measurement being taken, not necessarily any known condition/factor involved in the experiment
    - chemical [CHEBI](https://www.ebi.ac.uk/ols4/ontologies/chebi/classes/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252FCHEBI_24431?lang=en)
    - diet [EFO](https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0002755)
    - temperature [EFO:0001702](https://www.ebi.ac.uk/ols4/ontologies/efo/classes?obo_id=EFO%3A0001702)
    - protein [UniProt](https://www.uniprot.org/) submitted as `uniprot:P28906`
    - antibody [UniProt](https://www.uniprot.org/) submitted as `anti-uniprot:P28906`
    - multiple <code>&nbsp;||&nbsp;</code>-delimited terms may be used if appropriate
    - `na` for controls or baseline conditions
- **Embeddings in obsm**:
...
- **Additional standards for single-puck Slide-seq/Curio datasets**:
  - **obsm['spatial']**
  - multiple-puck datasets are permitted if each puck is also submitted individually
...
- **Additional genetic perturbation standards**
  - currently only supported for human, mouse, or zebrafish
  - **uns['genetic_perturbations']** dictionary
    - see [the schema](https://chanzuckerberg.github.io/single-cell-curation/latest-schema.html#genetic_perturbations) for specifics
  - **obs['genetic_perturbation_id']**
    - identifer(s) corresponding to the uns['genetic_perturbations'] key(s)
    - multiple <code>&nbsp;||&nbsp;</code>-delimited terms may be used if appropriate
    - `na` for the lack of genetic perturbations
  - **obs['genetic_perturbation_strategy']**
    - `CRISPR activation screen`, `CRISPR interference screen`, `CRISPR knockout mutant`, or `CRISPR knockout screen`
    - `control` if the uns['genetic_perturbation']['id']['role'] value(s) corresponding to all the relevant obs['genetic_perturbation_id'] are `control`
    - `no perturbations` if corresponding genetic_perturbation_id is `na`
