...
- **Cell metadata in obs (the values MUST be the most specific specific term available from the specified ontology)**:
  ...
  - self_reported_ethnicity_ontology_term_id: [HANCESTRO](https://www.ebi.ac.uk/ols/ontologies/hancestro) if human, `unknown` if information unavailable, `na` if non-human
