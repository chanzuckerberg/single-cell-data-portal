...
type CategoryKey = keyof Record<CATEGORY_KEY, string>;

// Set of all category values keyed by their corresponding category, for the full result set.
type CategorySet = { [K in CATEGORY_KEY]: Set<CategoryValueKey> };

// Metadata value and corresponding count.
...
// type FilterState = Map<CategoryKey, Map<CategoryValueKey, CategoryValue>>;
type FilterState = {
...
export function useFacetedFilter(
  ...
  }, [filters, originalRows, categorySet]);
  ...
  return {
    categories: buildCategoriesView(filterState),
    onFilter,
  };
}
 ...
 */
function buildCategoriesView(filterState?: FilterState): Category[] {
  if (!filterState) {
    return [];
  }
  return Object.keys(filterState)
    .map((categoryKey: string) => {
      ...
      const sortedCategoryValues = [...categoryValueByValue.values()].sort(
        sortCategoryValues
      );
      return {
        key: categoryKey as CategoryKey,
        values: sortedCategoryValues,
      };
    })
    .sort(sortCategories);
}
 ...
 * @param categoryValue - Category value to toggle selected state of.
...
/**
 * Sort categories by key, ascending.
 ...
 */
function sortCategories(c0: Category, c1: Category): number {
  if (c0.key < c1.key) {
    return -1;
  }
  if (c0.key > c1.key) {
...
/**
 * Sort category values by key, ascending.
 * @param cv0 - First filtered rows to compare.
 * @param cv1 - Second filtered rows to compare.
 * @returns Number indicating sort precedence of cv0 vs cv1.
 */
function sortCategoryValues(cv0: CategoryValue, cv1: CategoryValue): number {
  if (cv0.key < cv1.key) {
    return -1;
  }
  if (cv0.key > cv1.key) {
...
function summarizeCategory(
  ...
  return filteredRows.reduce(
    (
      ...
      // Init category value it doesn't already exist. Init count to 0, set selected flag from filter state.
      categoryValueKeys.forEach((categoryValueKey: CategoryValueKey) => {
        ...
        // Increment count for this category value key.
        categoryValue.count = categoryValue.count + 1;
      });
      return accum;
    },
