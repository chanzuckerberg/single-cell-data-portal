...
export const downloadMultipleFiles = (formatsToDownload: DATASET_ASSET_FORMAT[], downloadLinks: DownloadLinkType[]) => {
...
export const getDownloadLink = async (asset: DatasetAsset): Promise<DownloadLinkType> => {
  try {
    ...
    const result = await fetch(`${API_URL}${url}`, { ...DEFAULT_FETCH_OPTIONS, method: "GET" }).then((res) =>
      res.json()
    );

    return { filetype: asset.filetype, fileSize: result.file_size, downloadURL: result.url };
  } catch (error) {
    console.error("Error fetching download link for asset:", asset, error);
    return { filetype: asset.filetype, fileSize: undefined, downloadURL: undefined };
  }
};