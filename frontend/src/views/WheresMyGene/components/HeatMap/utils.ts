...
/**
 * Value format: `${viewId}~${tissue}~${name}~${depth}`
 */
...
export function getAllSerializedCellTypeMetadata(
  ...
  return cellTypes.map(({ viewId, name, depth, total_count }) => {
    return `${viewId}~${tissue}~${name}~${depth}~${total_count}` as CellTypeMetadata;
  });
}
...
export function deserializeCellTypeMetadata(
  ...
  const [rawViewId, tissue, name, depth, total_count] = cellTypeMetadata.split(
