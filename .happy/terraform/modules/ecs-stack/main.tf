...
locals {
  ...
  frontend_cmd                 = []
  backend_cmd                  = ["gunicorn", "--worker-class", "gevent", "--workers", "8", "--bind", "0.0.0.0:5000", "backend.api.data_portal.api_server.app:app", "--max-requests", "10000", "--timeout", "30", "--keep-alive", "5", "--log-level", "info"]
