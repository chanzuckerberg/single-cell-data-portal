...
class TestDatasetASD(BaseAuthAPITest, CorporaTestCaseUsingMockAWS):
    def setUp(self):
        # Needed for proper setUp resolution in multiple inheritance
        super().setUp()
    ...
    def test__get_all_datasets_for_index(self):
        ...
        response = self.app.get(test_url.url, headers=headers)
        self.assertEqual(200, response.status_code)
        # Note: testing the payload is difficult because this will retrieve datasets generated by previous
        #  tests.
