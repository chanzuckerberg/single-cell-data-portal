...
class TestDataset(BaseAuthAPITest, CorporaTestCaseUsingMockAWS):
    ...
    def test__get_all_datasets_for_index(self):
        dataset = self.generate_dataset(
            self.session, id="test_dataset_id_for_index", cell_count=42, published_at=datetime.now()
        )
        ...
        self.assertEqual(actual_dataset["published_at"], dataset.published_at)
        self.assertEqual(actual_dataset["revised_at"], dataset.revised_at)
