...
class TestAuthToken(BaseAPITest):
    ...
    def test_post_token(self, auth0_management_session, CorporaAuthConfig):
        ...
        token = self.app.post("/curator/v1/auth/token", headers={"x-api-key": user_api_key}).json()["access_token"]
    ...
    @patch("backend.corpora.lambdas.api.v1.auth.keys.CorporaAuthConfig")
    def test_post_token(self, CorporaAuthConfig):
        test_secret = "password1234"
        test_user_id = "test_user_id"
        CorporaAuthConfig().api_key_secret = test_secret
