...
class TestKeys(BaseAuthAPITest):
    ...
    @patch.multiple(
        "backend.corpora.api.v1.auth.keys.auth0_management_session",
        get_user_api_key_identity=DEFAULT,
        store_api_key=DEFAULT,
        link_api_key=DEFAULT,
        delete_api_key=DEFAULT,
    )
    @patch.multiple("backend.corpora.api.v1.auth.keys", CorporaAuthConfig=DEFAULT, get_userinfo=DEFAULT)
    ...
    @patch("backend.corpora.api.v1.auth.keys.auth0_management_session.get_user_api_key_identity")
