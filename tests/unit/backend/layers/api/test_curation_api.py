...
class TestGetCollectionVersionID(BaseAPIPortalTest):
    ...
    def test_get_collection_version_ok(self):
        ...
        self.business_logic.publish_collection_version(revision.version_id)
        res = self.app.get(f"/curation/v1/collection_versions/{first_version.version_id}", headers=self.make_owner_header())
        self.assertEqual(200, res.status_code)
    ...
    def test_get_collection_version_403(self):
        with self.subTest("Query endpoint with incorrect ID"):
            res = self.app.get(f"/curation/v1/collection_versions/{str(uuid.uuid4())}",
                               headers=self.make_owner_header())
            self.assertEqual(403, res.status_code)
        with self.subTest("Query endpoint with Canonical ID"):
            collection = self.generate_published_collection()
            res = self.app.get(f"/curation/v1/collection_versions/{collection.collection_id}",
                               headers=self.make_owner_header())
            self.assertEqual(403, res.status_code)
        with self.subTest("Query endpoint with non-UUID"):
            res = self.app.get("/curation/v1/collection_versions/bad-input-id",
                               headers=self.make_owner_header())
            self.assertEqual(403, res.status_code)
