...
class TestExtract(CorporaTestCaseUsingMockAWS):
    ...
    def test_get_s3_uris_pulls_expected_datasets(self):
        ...
        not_expected_datasets = [self.dataset__tombstoned, self.dataset__not_primary, self.dataset__not_published,
                                 self.dataset__private_collection, self.dataset__wrong_assay]
        ...
        for uri in expected_s3_uris:
            self.assertIn(s3_uris, uri)
        for uri in not_expected_s3_uris:
            self.assertNotIn(s3_uris, uri)
