...
def create_temp_cube(dim_size=3,
                     attr_vals_fn: Callable[[List], Dict[str, list]] = random_attr_values) -> None:
...
def create_cube(cube_dir,
    ...
    with tiledb.open(cube_dir, mode='w') as cube:
        ...
        coords = [[all_dims_domain_values[i_dim][(i_row // dim_size ** i_dim) % dim_size]
                   for i_row in range(n_coords)]
        ...
        physical_attr_values = {cube_logical_dims[i]: coords[i]
                                for i in range(len(cube_indexed_dims), len(cube_logical_dims))}
