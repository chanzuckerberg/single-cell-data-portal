...
def run_cellguide_pipeline():
    ...
    with unittest.mock.patch("backend.wmg.data.snapshot.load_snapshot", new=custom_load_snapshot):
        ...
        shutil.move(
            f"{output_directory}/{CELL_GUIDE_TISSUE_METADATA_FILENAME}", f"{output_directory}/tissue_metadata.json"
        )
        shutil.move(
