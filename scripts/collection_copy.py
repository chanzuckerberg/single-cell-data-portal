...
def cli(ctx, deployment):
    ...
    api_base = {"dev": "https://api.dev.single-cell.czi.technology/db/v1",
...
def copy_collection(ctx, uuid, cookie):
    """Copy a collection from prod to the passed in env."""
    ...
    response = requests.post(f"{ctx.obj['api_base']}/collections/", headers=headers, data=json.dumps(collection_metadata))


    new_collection_uuid = json.loads(response._content)["collection_uuid"]

    for asset in dataset_assets:
        response = requests.post(f"https://api.cellxgene.cziscience.com/dp/v1/datasets/{asset['dataset_id']}/asset/{asset['asset_id']}")
        presigned_s3_uri = json.loads(response._content)['presigned_url']
        dataset_body = {
            "dataset_uuid": asset["dataset_id"],
            "url": presigned_s3_uri
        }
        response = requests.post(f"{ctx.obj['api_base']}/collections/{new_collection_uuid}/upload-links", headers=headers, data=json.dumps(dataset_body))
        dataset_uuid = json.loads(response._content)
        click.echo(f"Original_dataset_uuid: {asset['dataset_id']}, new dataset_uuid: {dataset_uuid}")
