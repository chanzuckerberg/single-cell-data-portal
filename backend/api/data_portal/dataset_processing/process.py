...
from backend.api.data_portal.common.entities import Dataset, DatasetAsset
...
def main():
    ...
    try:
        if step_name == "download-validate":
            from backend.api.data_portal.dataset_processing.process_download_validate import process

            process(dataset_id, os.environ["DROPBOX_URL"], os.environ["ARTIFACT_BUCKET"])
        elif step_name == "cxg":
            from backend.api.data_portal.dataset_processing.process_cxg import process

            process(dataset_id, os.environ["ARTIFACT_BUCKET"], os.environ["CELLXGENE_BUCKET"])
        elif step_name == "seurat":
            from backend.api.data_portal.dataset_processing.process_seurat import process
