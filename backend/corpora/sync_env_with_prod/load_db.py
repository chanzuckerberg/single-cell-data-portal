...
def lambda_handler(event, context):
    load_db(CorporaDbConfig().database_uri,
            event.get("db_dump_s3_bucket"),
            event.get("db_dump_s3_key"))
...
def load_db(db_uri, db_dump_s3_bucket, db_dump_s3_key):
    ...
    update_env_specific_urls(db_uri,
                             explorer_host=f"cellxgene.{env}.single-cell.czi.technology",
                             deploy_env=env)
    print(f"Successfully loaded database with prod data")
...
def load_rds_data(db_uri, prod_sql_file):
    ...
    load_command = f"PGPASSWORD={db_cxn_args['password']} " \
                   f"psql " \
                   f"--dbname={db_name} " \
...
def parse_db_connection_args(db_uri):
    ...
    return dict(user=match.group('user'),
