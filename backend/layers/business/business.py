...
class BusinessLogic(BusinessLogicInterface):
    ...
    def get_collections(self, filter: CollectionQueryFilter) -> Iterable[CollectionVersion]:
        ...
        def predicate(version: CollectionVersion):
            ...
            schema_version = filter.schema_version is None or self._is_schema_version_match(
                version.schema_version, filter.schema_version
            )
            return published and owner and curator and schema_version
    ...
    def _filter_private_collection_versions_by_schema_version(
        ...
        """
        Returns a list of private collection versions that match the given schema_version; result set generated by checking the schema
        version of datasets in each collection version.
        :param schema_version_pattern: The schema version to filter by.
        :param collection_versions: The list of private collection versions to filter.
        ...
        for collection_version in collection_versions:
            ...
            filtered_dataset_versions = [
                dv
                for dv in dataset_versions
                if dv.metadata is not None
                and self._is_schema_version_match(dv.metadata.schema_version, schema_version_pattern)
            ]
    ...
    def _is_schema_version_match(self, schema_version: str, schema_version_pattern: str) -> bool:
        """
        Determines if the schema version is a wildcard match if the schema version pattern contains wildcards, otherwise
        checks for an exact match.
        :param schema_version: The schema version to check.
        :param schema_version_pattern: The schema version pattern to check against. May contain wildcards.
        :return: True if the schema version matches the pattern, False otherwise.
        """
