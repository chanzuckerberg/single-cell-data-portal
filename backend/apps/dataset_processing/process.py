...
from backend.apps.common.entities import Dataset, DatasetAsset
from backend.apps.common.utils.db_helpers import processing_status_updater
from backend.apps.common.utils.db_session import db_session_manager
from backend.apps.common.utils.dl_sources.url import from_url
from backend.apps.common.utils.s3_buckets import buckets
from backend.apps.dataset_processing.download import download
from backend.apps.dataset_processing.exceptions import ProcessingCancelled, ProcessingFailed, ValidationFailed
from backend.apps.dataset_processing.h5ad_data_file import H5ADDataFile
from backend.apps.dataset_processing.slack import format_slack_message
...
def process(dataset_id, dropbox_url, cellxgene_bucket, artifact_bucket):
    ...
    # To implement proper cleanup, tests/unit/backend/apps/dataset_processing/test_process.py
...
def main():
    ...
    try:
        if step_name == "download-validate":
            from backend.apps.dataset_processing.process_download_validate import process

            process(dataset_id, os.environ["DROPBOX_URL"], os.environ["ARTIFACT_BUCKET"])
        elif step_name == "cxg":
            from backend.apps.dataset_processing.process_cxg import process

            process(dataset_id, os.environ["ARTIFACT_BUCKET"], os.environ["CELLXGENE_BUCKET"])
        elif step_name == "seurat":
            from backend.apps.dataset_processing.process_seurat import process
