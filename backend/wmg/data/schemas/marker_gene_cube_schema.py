...
marker_genes_attrs = [
    tiledb.Attr(name="gene_ontology_term_id", dtype="ascii", var=True, filters=filters_categorical),
    tiledb.Attr(name="marker_score", dtype=np.float32, filters=filters_numeric),
    tiledb.Attr(name="specificity", dtype=np.float32, filters=filters_numeric),
]

marker_genes_domain = tiledb.Domain(
    [
        tiledb.Dim(name=marker_genes_indexed_dim, domain=None, tile=None, dtype="ascii", filters=filters_categorical)
