...
def build_tissues_types(tissue_term_ids: List[str]) -> Dict[str, List]:
    return dict(
            [(tissue_term_id, build_viz_dots(cell_type_term_ids)) for tissue_term_id in tissue_term_ids]
    )


def build_genes(gene_term_ids: List[str], tissue_term_ids: List[str]) -> Dict[str, dict]:
    return dict([(gene_term_id, build_tissues_types(tissue_term_ids)) for gene_term_id in gene_term_ids])
