...
from backend.wmg.pipeline.utils import (
    decorate_anndata_with_assays_to_filter_out,
...
def apply_pre_concatenation_filters(anndata_object: anndata.AnnData, min_genes: int = None) -> anndata.AnnData:
    ...
    decorate_anndata_with_lowly_covered_cells_to_filter_out(anndata_object, min_genes)
    logger.info("Finished capturing lowly-covered cells")

    # Filter out cells generated by assays that dont provide gene length normalization
    assay_ontology_ids_to_keep = list(INCLUDED_ASSAYS.keys())
    decorate_anndata_with_assays_to_filter_out(anndata_object, assay_ontology_ids_to_keep)
    logger.info("Finished capturing cells generated by assays without gene length normalization")
