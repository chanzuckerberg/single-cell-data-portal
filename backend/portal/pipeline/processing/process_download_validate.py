...
from backend.portal.pipeline.processing.download import download

from backend.portal.pipeline.processing.exceptions import ValidationFailed
from backend.portal.pipeline.processing.common import create_artifact, update_db, download_from_s3, get_bucket_prefix
from backend.portal.pipeline.processing.logger import logger
...
def extract_metadata(filename) -> dict:
    ...
    metadata = {
        ...
        "donor_id": adata.obs["donor_id"].unique().tolist(),
        "suspension_type": adata.obs["suspension_type"].unique().tolist(),
    }
