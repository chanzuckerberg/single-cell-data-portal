"""Create the 'remix' dataset for the Krasnow Lab Human Lung Cell Atlas, smartseq-2. The cell
type and cell ontology was manually curated and can be seen in the cell_type_map and
...
def remix(adata, title: str):
    ...
    adata.obs["assay_ontology"] = "EFO:0008442"
    ...
    adata.uns["project_name"] = "A molecular cell atlas of the human lung from single cell RNA sequencing"
    adata.uns["project_description"] = (
        "Droplet- and plate-based single cell RNA sequencing applied to "
    ...
    adata.obs["cell_type"] = adata.obs["free_annotation"].str.split("_", expand=True)[0].map(cell_type_map)
    ...
    cell_type_ontology_map = {
        ...
        "Proximal Ciliated": "",
        "Goblet": "CL:1000143",
        "Dendritic": "CL:0001056",
        "Neutrophil": "CL:0000775",
    }

    adata.obs["cell_type_ontology"] = (
        adata.obs["free_annotation"].str.split("_", expand=True)[0].map(cell_type_ontology_map)
    )
    ...
    merged_raw_counts = pd.DataFrame(adata.raw.X, index=adata.obs.index, columns=upgraded_var_index).sum(
    ...
    sc.pp.normalize_total(remix_adata, target_sum=1e6)
...
# Process h5ad
ad = sc.read_h5ad("krasnow_lab_human_lung_cell_atlas_smartseq2-2-curated.h5ad")
basic_curation(ad)
print_summary(ad)
ad.write("krasnow_lab_human_lung_cell_atlas_smartseq2-2-curatedv2.h5ad", compression="gzip")
rad = remix(ad, title="Krasnow Lab Human Lung Cell Atlas, Smart-seq2")
print_summary(rad)
rad.write("krasnow_lab_human_lung_cell_atlas_smartseq2-2-remixed.h5ad", compression="gzip")
