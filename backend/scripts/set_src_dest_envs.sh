...
# allow dev->rdev for creating test environments
  ...
  if [[ -z "$RDEV_ENV" ]]; then
    ...
    exit 1
      ...
      read -n 1 -p "ATTENTION: You are mirroring the dev env to rdev/${RDEV_ENV}. Useful for testing fixes in dev for data-related bugs found in staging, but otherwise useless. Proceed? (Y/n) " ANS
      [[ $ANS == 'Y' ]] || exit 1
  fi
