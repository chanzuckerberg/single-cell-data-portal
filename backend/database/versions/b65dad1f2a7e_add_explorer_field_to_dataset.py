...
def upgrade():
    ...
    op.execute(
        "UPDATE dataset SET explorer_url=deployment_directory.url FROM deployment_directory WHERE deployment_directory.dataset_id=dataset.id;"
    )  # noqa E501
...
def downgrade():
    ...
    op.execute(
        "INSERT into deployment_directory (id, dataset_id, url, created_at, updated_at) SELECT uuid_generate_v4(), id, explorer_url, NOW(), NOW() FROM dataset WHERE dataset.explorer_url is not null;"
    )  # noqa E501
