...
for index, row in projects_df.iterrows():
    ...
    engine.execute(f"INSERT INTO project VALUES ('{project_id}', '{title}', '{label}', '{description}', '{category}', '{developmental_stage}', '{disease_ontology}', '{sample_type}', '{organ_part}', '{analysis_protocol}', {cell_count}, {donor_count}, '{publication_title}', '{publication_doi}', '{contact_name}', '{contact_institution}', '{contact_email}')")

    # library prep protocol
    engine.execute(f"INSERT INTO library_prep_protocol VALUES (NULL, '{library_construction_method_ontology}', '{end_bias}', '{nucleic_acid_source}')")
    ...
    # lpp x project
    engine.execute(f"INSERT INTO library_prep_protocol_join_project VALUES (NULL, 1, '{project_id}')")
...
files_df.insert(1, "project_id", ["HCA-Project-cddab57b-6868-4be4-806f-395ed9dd635a"] * files_df.shape[0])
...
for i in range(num_chunks):
    chunk = items[i * chunk_size:len(items) if len(items) < i * chunk_size + chunk_size else i * chunk_size + chunk_size]
